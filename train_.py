...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    target_train_loader, target_dataset = create_dataloader(train_path, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
                                              hyp=hyp, augment=True, cache=opt.cache, rect=opt.rect, rank=LOCAL_RANK,
                                              workers=workers, image_weights=opt.image_weights, quad=opt.quad,
                                              prefix=colorstr('train: '))
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        target_pbar = enumerate(target_train_loader)
        ...
        # （1）burn up 部分的源域数据和学生网络进行训练，现在设置初始epoch为50
        if epoch < 299 :
        ...
        if epoch > 9950: 
            ...
            for i, (imgs, targets, paths, _) in target_pbar:  # batch -------------------------------------------------------------
                ni = i + nb * epoch  # number integrated batches (since train start)
                imgs = imgs.to(device, non_blocking=True).float() / 255.0  # uint8 to float32, 0-255 to 0.0-1.0
                ...
                with amp.autocast(enabled=cuda):
                    ...
                    # 教师模型；送入弱增强的目标域无标签图像
                    with torch.no_grad():
                        teacher_pred = teacher_model(imgs)[0]  # forward                        
                    # 对教师网络的预测做NMS，用于伪标签
                    teacher_pred = non_max_suppression(teacher_pred, conf_thres = 0.25, iou_thres = 0.45, classes = None, agnostic = False, max_det=1000)
                    img_origin_shape = (1024, 2048, 3)
                    det = labels2targets(teacher_pred, imgs[2:],img_origin_shape,imgs)
                    pred = model(imgs)  # forward
                    loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
                    if RANK != -1:
                        loss *= WORLD_SIZE  # gradient averaged between devices in DDP mode
